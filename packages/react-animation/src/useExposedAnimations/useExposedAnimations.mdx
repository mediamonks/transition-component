...
The `useExposedAnimations` hook is used to get a collection of animations which are exposed in the global animation
Map for the given references (not necessarily an HTMLElement). The hook will trigger a rerender when
the animation for any of the references in the collection changes, is added, or is removed.
